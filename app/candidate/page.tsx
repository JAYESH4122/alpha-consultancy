...
export default function CandidateOverview() {
  ...
  const matchedJobRecord = jobs.find((job) => job.id === recentMatch?.jobId);
  const matchedJob = matchedJobRecord ? toEmployeeJobPreview(matchedJobRecord) : undefined;

  return (
    <>
      <PageHeader title={`Good morning, ${candidate.name.split(" ")[0]}`} description="Your identity stays private until you approve sharing it for an interview." action={<Link className="button button-primary" href="/employee/matches">View matches <ArrowRight size={17} /></Link>} />
      ...
      <div className="dashboard-grid">
        <section className="panel panel-large">
          <div className="panel-heading"><div><h2>Best match for you</h2><p>Based on your category, skills, city, experience, and shift.</p></div><Link href="/employee/matches">All matches</Link></div>
          {matchedJob && recentMatch ? <div className="featured-job">
            ...
            <div className="featured-job-main"><div className="featured-job-title"><span className="company-placeholder">Employer protected</span><h3>{matchedJob.title}</h3><p><MapPin size={15} /> {matchedJob.locationLabel} · {matchedJob.shift} shift</p></div><div className="salary">₹{(matchedJob.salaryMin / 1000).toFixed(0)}k–₹{(matchedJob.salaryMax / 1000).toFixed(0)}k <span>/ month</span></div></div>
            ...
            <Link className="button button-dark" href="/employee/matches">Review opportunity <ArrowRight size={16} /></Link>
        ...
        <section className="panel">
          ...
          <Link className="button button-secondary button-full" href="/employee/profile">Review profile</Link>
        </section>
      </div>
