...
export function PhoneAuth() {
  ...
  return <section className="production-auth">
    ...
    {phase === "phone" ? <div className="auth-fields"><label>Account type<select value={role} onChange={(event) => setRole(event.target.value as "candidate" | "employer")}><option value="candidate">Employee</option><option value="employer">Employer</option></select></label><label>Mobile number<input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+91 98765 43210" inputMode="tel" /></label><button className="button button-primary" type="button" disabled={pending || phone.length < 10} onClick={sendOtp}>{pending ? "Sending…" : "Send OTP"}</button></div> : <div className="auth-fields"><label>Verification code<input value={token} onChange={(event) => setToken(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" placeholder="6-digit code" /></label><button className="button button-primary" type="button" disabled={pending || token.length !== 6} onClick={verifyOtp}>{pending ? "Checking…" : "Verify & continue"}</button></div>}
